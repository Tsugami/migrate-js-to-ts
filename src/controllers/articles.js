  ...
  try {
    ...
    if (data.tagList) {
      for (const t of data.tagList) {
        const tagExists = await Tag.findByPk(t);
        let newTag;
        if (!tagExists) {
...
module.exports.deleteArticle = async (req, res) => {
  try {
    const slugInfo = req.params.slug;
    const article = await Article.findByPk(slugInfo, { include: Tag });
...
module.exports.getAllArticles = async (req, res) => {
  try {
    ...
    for (const t of article) {
      const addArt = sanitizeOutputMultiple(t);
...
module.exports.getFeed = async (req, res) => {
  try {
    ...
    const followingUserEmail = [];
    for (const t of followingUsers[0]) {
